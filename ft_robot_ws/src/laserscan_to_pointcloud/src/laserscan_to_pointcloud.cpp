...
ScanToPointCloud::ScanToPointCloud() 
{
 ...
 //雷达安装和小车没有对其,选择了180这里没有用坐标旋转,需要进一步改进
    ...
    publisher = nh.advertise<std_msgs::Int32>("int_point", 10);
...
//车后滤波
    ...
    std::cout<<" cloud_msg->points.size() = "<< cloud_msg->points.size()<<","<<cloud_msg1->points.size()<<","<<cloud_msg2->points.size()<<std::endl;
    size_t pointCount2 = cloud_msg2->size();
    size_t pointCount1 = cloud_msg1->size();
    std::cout<<"障碍物1:"<<pointCount1<<"障碍物2:"<<pointCount2<<std::endl;
    std_msgs::Int32 msg;
    if (pointCount2 > pointCount1)
    {
        msg.data = pointCount2;
    }
    else
    {
        msg.data = pointCount1;
    }
