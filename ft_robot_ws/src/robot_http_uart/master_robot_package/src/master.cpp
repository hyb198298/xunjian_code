...
void Master::nav_worker_thread()
 {
    while (open_start)
    {
        ...
        if (nav_vel == true)
        {
            ...
            {
                double cost = std::sqrt(std::pow(all_rout[i][0] - all_rout[i+1][0], 2) + std::pow(all_rout[i][1] - all_rout[i+1][1], 2));
                Length+=cost;
            ...
            double  num =  (this->all_rout.size() - 1)/Length;
            int temp=1;
            if(num>1)
            {
                num=1;
            ...
            std::cout<<"向前预瞄个数:"<<temp<<"每个点的长度"<<num<<std::endl;
            while (true)
            {
...
void  Master:: pub_point()
{
    ...
    std::cout << "原始路径以发布" << std::endl;
}

void  Master:: pub_pointr()
{
    nav_msgs::Path path;
    std::cout << "发布剔除冗杂点后路径" << std::endl;
    for (auto it = all_rout.begin(); it != all_rout.end(); it++)
    {
        ros::Time current_time, last_time;
        ...
        this_pose_stamped.pose.orientation.y = qtn.getY();
        this_pose_stamped.pose.orientation.z = qtn.getZ();
        this_pose_stamped.pose.orientation.w = qtn.getW();
        path.poses.push_back(this_pose_stamped);
    }
    path_pubr.publish(path);
    std::cout << "发布剔除冗杂点后路径发送完毕" << std::endl;
}
...
void  Master::get_path()
{
    ...
    int     size_numm=n/100;
     std::cout<<"size_numm"<<size_numm<<std::endl;
    if(size_numm==0)
    {
        size_numm=200;
    }
        ...
        size_numm=size_numm*400;
    }
    std::cout<<"1数量"<<size_numm<<std::endl;
    double step= 1.0/size_numm;
    std::cout<<"step"<<step<<std::endl;
    for (double u = 0.0; u < 1; u +=step)
    {
        for (int i = 0; i <= n; i++)
        {
    ...
    std::cout << "Matrix P:\n"
              << Bik_fuc << std::endl;
    std::cout << "控制点数量" << path_y.size() << std::endl;
    std::cout << "B样条数量" << size_numm << std::endl;
    // 求解路径点曲率
    std::vector<double> p_x;
    std::vector<double> p_y;
    std::vector<double> heading;
    std::cout << "1数量hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh" << std::endl;
