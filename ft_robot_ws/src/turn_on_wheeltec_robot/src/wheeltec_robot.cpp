...
void turn_on_robot::Publish_ImuSensor()
{
  ...
  //-------------------------------------------------------------------
   tf::Quaternion quat(Mpu6050.orientation.x,Mpu6050.orientation.y,Mpu6050.orientation.z,Mpu6050.orientation.w);
   double roll,pitch,yaw;
   tf::Matrix3x3(quat).getRPY(roll,pitch,yaw);
  //  std::cout<<"yaw = "<<yaw*57.29578<<std::endl;
       static float yaw_bak = 0;;
      float dyaw=  yaw -yaw_bak;
      if(fabs(dyaw)>2)
      {
        std::cout<<"yaw ================== "<<yaw<<","<<yaw_bak<<std::endl;
      }
      yaw_bak = yaw;

//   geometry_msgs::Quaternion q;
//   q.x = Mpu6050.orientation.x; //A quaternion represents a three-axis attitude //四元数表达三轴姿态
//   q.y = Mpu6050.orientation.y; 
//   q.z = Mpu6050.orientation.z;
//   q.w = Mpu6050.orientation.w;
//   tf2::Quaternion imu_quat;
//   tf2::convert(q,imu_quat);
  
// tf2::Matrix3x3 mat( imu_quat);
//   double roll,pitch,yaw;
//   // imu_quat.getRPY(roll,pitch,yaw);
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
                                   ...
                                   //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      float z = Robot_Pos.Z;
      // z+=Robot_Vel.Z * Sampling_Time; 
      z+=Mpu6050.angular_velocity.z * Sampling_Time; 
      if(z>3.1415926)
        z -= 3.1415926*2;
      else if(z<=-3.1415926)
        z += 3.1415926*2;
      ...
      static float z_bak = 0;;
      float dz =  Robot_Pos.Z -z_bak;
      if(fabs(dz)>2)
      {
        std::cout<<"Robot_Pos.Z ==========+++++++++++++++++============= "<<Robot_Pos.Z<<","<<z_bak<<std::endl;
      }
      z_bak = Robot_Pos.Z;
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  odom_publisher    = n.advertise<nav_msgs::Odometry>("odom1", 50); //Create the odometer topic publisher //创建里程计话题发布者
